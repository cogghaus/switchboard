...
|--------------------|--------------------|-----------------|--------|
| reviewer-security  | Security Reviewer  | claude-opus-4-8 | opus   |
| testing-red-team   | Red Team Lead      | claude-opus-4-8 | opus   |
| testing-pentester  | Infra Pentester    | claude-opus-4-8 | opus   |
...
## Notes
- Model surfaces: a sub-agent definition's model: frontmatter accepts a full model ID; the live Agent tool takes only aliases {fable, opus, sonnet, haiku}. Aliases move between model versions (the opus alias was observed as Opus 5 on 2026-08-25 and as Opus 4.8 on 2026-08-26), so never rely on an alias to reach a security-critical tier - pin claude-opus-4-8 by full ID in the definition and re-verify alias behavior periodically.
...
- Frontier-tier allowance discipline: the frontier tier (Fable) draws down its Max allowance ~2x faster than Opus and is capped weekly. Reserve it for tiers where its ambiguity/design/orchestration strength compounds (design/spec, long orchestration, brainstorm, architect, designer, loki). Do not route mechanical or logic-bound work to it.
